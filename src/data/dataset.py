...
class EpiDataset(Dataset):
    def __init__(
        ...
        self.id_to_country = {i: c for i, c in enumerate(countries)}
    ...
    def _build_windows(self, df, country_col, date_col):
        for iso, group in df.groupby(country_col):
            ...
            while start + self.window_days <= n:
                ...
                times = np.linspace(0.0, 1.0, self.window_days,
                                    dtype=np.float32)
                self.samples.append({
                    "times":      times,
                    "values":     window_vals,
            ...
            # Outside while — capture final window
...
def make_dataloaders(
    ...
    log.info(f"Train: {df_train['date'].min().date()} → "
             f"{df_train['date'].max().date()} ({len(df_train):,} rows)")
    log.info(f"Val:   {df_val['date'].min().date()} → "
             f"{df_val['date'].max().date()} ({len(df_val):,} rows)")
